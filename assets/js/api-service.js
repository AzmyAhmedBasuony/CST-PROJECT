...
class ApiService {
    ...
    async fetchProductsByCategory(category) {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.products}/category/${category}`);
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
            const apiProducts = await response.json();
    ...
    async fetchProductById(id) {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.product}/${id}`);
            if (!response.ok) {
                throw new Error(`HTTP error! status: ${response.status}`);
            }
    ...
    mapApiProductsToLocal(apiProducts) {
        return apiProducts.map(apiProduct => this.mapApiProductToLocal(apiProduct));
    }
    ...
    async checkApiHealth() {
        try {
            const response = await fetch(`${this.baseUrl}${this.endpoints.products}?limit=1`);
            return response.ok;
        } catch (error) {
            return false;
        }
