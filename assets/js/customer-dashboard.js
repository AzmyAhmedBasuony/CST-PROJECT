...
function loadCustomerData() {
  ...
  customerOrders.forEach(customerOrder => {
    const matchingSellerOrder = sellerOrders.find(sellerOrder => sellerOrder.id === customerOrder.id);
    
    if (matchingSellerOrder && matchingSellerOrder.status !== customerOrder.status) {
      customerOrder.status = matchingSellerOrder.status;
      hasUpdates = true;
    }
  });
...
function updateDashboardStats() {
  ...
  if (totalOrdersElement)
    totalOrdersElement.textContent = customerOrders.length;
...
function loadOrders() {
  ...
  container.innerHTML = customerOrders
    .map(
      (order) => `
        <div class="card mb-3">
            <div class="card-body">
                <div class="row align-items-center">
                    <div class="col-md-3">
                        <h6 class="mb-1" style="color: #000 !important;">Order #${
                          order.id
                        }</h6>
                ...
                <div class="mt-3">
                    <small class="text-muted">
                        ${order.items.length} item(s) • Shipped to: ${
...
function loadProfile() {
  ...
  const totalSpent = customerOrders
    .filter((order) => order.status === "completed")
    .reduce((sum, order) => sum + order.total, 0);
  document.getElementById("profile-spent").textContent = `$${totalSpent.toFixed(
    2
  )}`;
}
                ...
                <div class="row align-items-center">
                    ...
                    <div class="col-md-2 text-end">
                        <div class="btn-group btn-group-sm">
                            <button class="btn btn-outline-primary" onclick="addToCartFromWishlist(${
                              item.id
                            })">
                                <i class="fas fa-cart-plus"></i>
                            </button>
                            <button class="btn btn-outline-danger" onclick="removeFromWishlist(${
                              item.id
                            })">
