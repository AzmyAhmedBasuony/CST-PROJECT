...
function loadSellerData() {
    ...
    sellerProducts = allProducts.filter(product => product.seller === currentUser.name);
    
    // Load orders (simulate orders for demo)
...
function updateDashboardStats() {
    ...
    if (totalProductsElement) totalProductsElement.textContent = sellerProducts.length;
...
function updateOrderStatus() {
    const orderId = document.querySelector('#order-details').getAttribute('data-order-id');
...
function loadRecentActivity() {
    ...
    const activities = [
        { type: 'order', message: 'New order #ORD003 received', time: '2 hours ago' },
        { type: 'product', message: 'Product "iPhone 15 Pro" stock updated', time: '4 hours ago' },
        { type: 'sale', message: 'Order #ORD002 completed', time: '1 day ago' },
        { type: 'product', message: 'New product "MacBook Air" added', time: '2 days ago' }
    ];

    activityContainer.innerHTML = activities.map(activity => `
        <div class="d-flex align-items-center mb-3">
            <div class="flex-shrink-0">
                <i class="fas fa-${activity.type === 'order' ? 'shopping-cart' : activity.type === 'product' ? 'box' : 'dollar-sign'} text-primary"></i>
            </div>
...
window.SellerDashboard = {
    initializeSellerDashboard, loadSellerData, updateDashboardStats, loadProductsTable,
    loadOrdersTable, filterProducts, filterOrders, showAddProductModal, editProduct,
    saveProduct, deleteProduct, viewOrderDetails, updateOrderStatus, initializeCharts,
    updateCharts, loadRecentActivity, logout
}; 